...
export class HeroService {
  ...
  getHero(id: number): Observable<Hero> {
    ...
    return this.http.get<Hero>(url).pipe(
      tap((_) => this.log(`toma el heroe con id ${id}`)),
      catchError(this.handleError<Hero>(`getHero id=${id}`))
    );
    //old
  ...
  updateHero(hero: Hero): Observable<any> {
    return this.http.put(this.heroesUrl, hero, this.httpOptions).pipe(
      tap((_) => this.log(`cambie el heroe id ${hero.id}`)),
      catchError(this.handleError<any>('updateHero'))
    );
  }

  /** POST: add a new hero to the server */
  // to InMemoryDataService DB via heroesUrl 'api/heroes'.
  // where it generates an id in method genId
  addHero(hero: Hero): Observable<Hero> {
    return this.http.post<Hero>(this.heroesUrl, hero, this.httpOptions).pipe(
      tap((newHero: Hero) =>
        this.log(`agregue al heroe ${newHero.name} id=${newHero.id}`)
      ),
  ...
  deleteHero(id: number): Observable<Hero> {
    ...
    return this.http.delete<Hero>(url, this.httpOptions).pipe(
      tap((_) => this.log(`borre el heroe id ${id}`)),
  ...
  searchHeroes(term: string): Observable<Hero[]> {
    ...
    return this.http.get<Hero[]>(`${this.heroesUrl}/?name=${term}`).pipe(
      tap((x) =>
        x.length
          ? this.log(`encontre estos heroes por "${term}"`)
          : this.log(`no hay nadie con "${term}"`)
      ),
